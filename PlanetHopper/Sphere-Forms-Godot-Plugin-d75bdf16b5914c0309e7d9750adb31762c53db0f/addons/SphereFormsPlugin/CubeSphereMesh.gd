@tool
@icon("CubeSphereMesh.svg")
## Class representing a spherical non-[PrimitiveMesh].
##
## Class that creates a [SphereMesh] with poligon coverage based on subdivided Cube. Good for triplanar textures.
class_name CubeSphereMesh extends ArrayMesh
#region ToolInterface
## Radius of cubesphere.
...
## Full height of the cubesphere
@export_range(0.001,100,0.001,"or_greater") var height := 1.0:
	set(value):
		if not height == value:
			height = value
			## Dunamic vertex updator
...
## Sets UV map type. By default Cubesphere has Square UV type. [br][br]
...
## Usualy meshes like Cubesphere has "subdivisions" parameter that splits cube triangle into 2^(subdivisions+1) triangles.[br][br]
## This parameter allow you to split cube triangle into edge_count^2 triangles for better "customization".
...
#region Constants
const A = 0.57735026918963
# base vertices
const core_vertices : PackedVector3Array = [
	Vector3(-A,-A,-A), Vector3(A,A,A)   , #0 ,1
	Vector3(A,-A,-A) , Vector3(A,A,-A)  , #2 ,3
	Vector3(-A,A,-A) , Vector3(-A,A,A)  , #4 ,5 
	Vector3(-A,-A,A) , Vector3(A,-A,A)  , #6 ,7 
	Vector3(A,-A,-A) , Vector3(A,A,-A)  , #2 ,3
]
# base triangles
const core_triangles: PackedVector3Array = [
	...
	Vector3(0,6,2),Vector3(7,2,6),Vector3(2,7,3),Vector3(1,3,7),
]
...
#region MeshBuild
# Changable variables
## Important array with mesh vertices. Required to edit height/radius of mesh without performance issues.[br][br]
...
func apply_size():
	...
	for i in vertices.size():
		new_vertices.push_back(Vector3(vertices[i].x * radius,vertices[i].y * height*0.5,vertices[i].z * radius))
	return new_vertices
...
func update_mesh(arrays:=[]):
	...
	if _uv_type==1:
		...
		for i in 3: # Five separate segments
			...
			uvs.append_array(square_uvs.duplicate().map(func(x):x.x = x.x+1/3.*i;return x))
		# Surface divider
		if edge_count>1:
		...
		vertices.resize(8)# required to remove last two (unneeded for sphere_uv mode) core_vertices
		...
		if _uv_type==0:
			for i in vertices.size():
				#vertices[i]=vertices[i].rotated(Vector3(-1,0,1).normalized(),deg_to_rad(55)) #zipper bug avoider (and UV displacer)
				vertices[i]=vertices[i].rotated(Vector3.UP,0.0001) #zipper bug avoider (and UV displacer)
			...
			for i in vertices.size():
				vertices[i]=vertices[i].rotated(Vector3.UP,-0.0001) #zipper bug avoider (and UV displacer)
				#vertices[i]=vertices[i].rotated(Vector3(-1,0,1).normalized(),-deg_to_rad(55)) #zipper bug avoider (return all verticess back in place)
				#vertices[i]=vertices[i].rotated(Vector3.LEFT,-deg_to_rad(45)) #zipper bug avoider (and UV displacer)
