...
func _update_mesh(arrays : Array, planet_data : PlanetData):
	print("_update_mesh called for face: ", name)
	var _mesh := ArrayMesh.new()
	_mesh.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, arrays)
	self.mesh = _mesh
	print("Mesh assigned to face: ", name, " vertex count: ", arrays[Mesh.ARRAY_VERTEX].size())
	
	# Temporary: use plain material to debug geometry
	var temp_mat := StandardMaterial3D.new()
	temp_mat.albedo_color = Color.WHITE
	material_override = temp_mat
